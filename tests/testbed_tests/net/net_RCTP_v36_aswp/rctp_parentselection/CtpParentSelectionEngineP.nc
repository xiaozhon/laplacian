  ...
  command error_t CtpParentSelectionConfig.addParentCandidate( routing_table_entry* entry, nx_uint16_t candPathETX)
  {
		...
		*/
		...
		if(prntSet.setSize == 0){
			// Adding the first candidate (CTP parent)
		...
		if(MAX_PARENTSET_SIZE > 1){
			if(prntSet.setSize < MAX_PARENTSET_SIZE){
				...
				if(candPathETX < prntSet.maxPathETX){
					...
					prntSet.maxPathETX = 0;
					prntSet.setETX = MAX_METRIC;
					for(i=0;i<prntSet.setSize;i++){  // Start from 1 to avoid the CTP parent
						if(parentSetPathETX[i] >  prntSet.maxPathETX){
							prntSet.maxPathETX = parentSetPathETX[i];
							prntSet.maxPathETXPos = i;
						}
						if(parentSetPathETX[i] <  prntSet.setETX){  // Find the lowest pathETX in the parent set and then compare with the CTP prnt
							prntSet.setETX = parentSetPathETX[i];
							prntSet.bestNeighbor = parentSetTable[i];		
						}
					}
					// Compare with the ctp parent
					//if(parentSetPathETX[0] <  prntSet.setETX){
					//	prntSet.setETX = parentSetPathETX[0];
					//	prntSet.bestNeighbor = parentSetTable[0];
					//}
