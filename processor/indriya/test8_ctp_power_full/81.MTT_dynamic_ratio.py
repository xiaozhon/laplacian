...
inf = open("../../../data/indriya/test8_ctp_power_full/cs_pkt_indriya_20161013_formatted.txt", "r")
...
#			print dt
			...
			for i in range(N):
				for j in range(N):
					...
					if union_matrix[i][j] > 0 and union_matrix[i][j] == union_matrix[j][i]:
						loop_count += 1
			if loop_count > 0:
				print str(round_count) + '\tloops: \t' + str(loop_count)
	#		print lm
			### matrix for matrix tree theorem
			### remove the isolated nodes from the laplacian matrix
			mtt = []	
...
#			print det
			...
			print 'out_degree product:\t' + str(K) + '\tdet\t' + str(det) + '\tdiff\t' + str(K - det)
...
#protocol, = ax.plot(protocol_trees, 'k', linestyle='dashed', marker='*', markersize=3, linewidth=1)

#env, = ax.plot(env_trees, 'k', linestyle='dashdot', marker = '+', markersize=3, linewidth=1)
...
x_ticks = [i for i in range(0, len(total_trees)+5, 50)]
...
x_ticks = [i for i in range(0, len(dynamic_ratio)+5, 50)]
ax.set_xticks(x_ticks)
ax.set_xticklabels(x_ticks,size=7)
...
x_ticks = [i for i in range(0, len(inter_trees)+5, 50)]
ax.set_xticks(x_ticks)
ax.set_xticklabels(x_ticks,size=7)
...
x_ticks = [i for i in range(0, len(inter_ratio)+5, 50)]
ax.set_xticks(x_ticks)
ax.set_xticklabels(x_ticks,size=7)
