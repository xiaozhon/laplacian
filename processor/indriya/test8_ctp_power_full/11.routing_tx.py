...
inf = open("../../../data/indriya/test8_ctp_power_full/summary_pkt_indriya_20161013_formatted.txt", "r")
...
### THE number of loops for each node 
...
#			print str(round_count) + ': ' + str(tmp)
			...
			tmp = []
			newline = 'nodeid\tpath_etx\n'
			newline += 'round: ' + str(round_count) + '\n'
			outf_etx.writelines(newline)
			for i in range(N):
				newline = '\t' + nodeids[i] + '\t' + str(last_petx[idx(nodeids[i])]) + '\n'
				outf_etx.writelines(newline)
				if last_petx[i] != 0:
					tmp.append(last_petx[i])
				
			
			newline = '\tavg: ' + str(np.mean(tmp)) + '\tstdev: ' + str(np.std(tmp)) + '\n'
	...
	if abs(cur_loop[idx(s[SRC])] - pre_loop[idx(s[SRC])]) > 100:
		#### current loop counter maybe wrong
		cur_loop[idx(s[SRC])] = pre_loop[idx(s[SRC])]
	
	last_reset[idx(s[SRC])] = int(s[TRICKLE])
	last_loop[idx(s[SRC])] = int(s[LOOP])
	last_petx[idx(s[SRC])] = int(s[PETX])
...
outf.writelines("total_loops sum(cycle_loop): %d\n" % (sum(cycle_loop)))
