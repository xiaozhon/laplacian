...
N = 97
TOTAL_CYCLES = 80
INTERVAL = 3	# two cycles

#inf = open("../../../data/indriya/test7_power_full/cs_pkt_indriya_20161015_formatted.txt", "r")
inf = open("../../../data/indriya/test13_prnt_3/20161101-rctp_indriya_all_packets.txt", "r")

inf_ids = open("../../../data/indriya/test13_prnt_3/nodeids.txt", "r")

inf_cycle_prr = open('../../../data/indriya/test13_prnt_3/node_prr_per_cycle.txt', 'r')
...
################### load cycle prr #############################################
### cycle overall information
### [avg, std]
cycle_prr = []

### each node's information
### idx = cycle_count, value = [prr, prnt_set]
...
#		print pm
		...
		## compute the out degree
			...
			if degrees[i] > 0:
				...
				prnt_set[i] = int(cycle_prr_per_nodes[round_count-1][i][1])
		### all links
		edges = sum(degrees)
...
protocol, = ax.plot([dr[i] * 100 for i in range(len(dr))], 'k', linestyle='solid', marker='*', markersize=1.5, linewidth=0.5)

env, = ax.plot([(1-dr[i])*100 for i in range(len(dr))], 'b', linestyle=':', marker = '+', markersize=1.5, linewidth=0.5)
