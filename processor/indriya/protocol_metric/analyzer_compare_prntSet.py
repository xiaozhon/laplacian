...
#### compare entropy
def compare_entropy(merge, f):
	inf = open('ctp_36nodes_noise/analysis_result_entropy_all.txt', 'r')
#	inf = open('ctp_36nodes_noise/analysis_result_entropy_no_loop_debug.txt', 'r')
	...
	for line in inf:
		...
		if line_count > 1:
			s = line.split()
			if len(s) > 0:
				entropies_ctp.append(float(s[1]))
				errs_ctp.append(float(s[2]))
	inf.close()

	inf = open('rctp_36nodes_noise/analysis_result_entropy_all.txt', 'r')
#	inf = open('rctp_36nodes_noise/analysis_result_entropy_no_loop_debug.txt', 'r')
	...
	for line in inf:
		...
		if line_count > 1:
			...
			if len(s) > 0:
				...
				entropies_rctp.append(float(s[1]))
				errs_rctp.append(float(s[2]))
	inf.close()

	print len(entropies_ctp)
	print len(entropies_rctp)

	x = np.arange(len(entropies_ctp))

	if not merge:
		plt.figure(figsize=(12, 3.5))

		plt.errorbar(x, entropies_ctp, yerr=errs_ctp, capsize=5, fmt='o-', label='ctp')
		plt.errorbar(x, entropies_rctp, yerr=errs_rctp, capsize=5, fmt='^-', label='ctp+eer')
		plt.legend()
		plt.title("Entropies v.s. noise")
		...
		plt.savefig('Entropy_vs_noise_level_ctp_vs_rctp.png', dpi=300)
		plt.clf()
	else:
		f.errorbar(x, entropies_ctp, yerr=errs_ctp, capsize=5, fmt='o-', label='ctp')
		f.errorbar(x, entropies_rctp, yerr=errs_rctp, capsize=5, fmt='^-', label='ctp+eer')
		f.legend()
		f.set_title("Entropies v.s. noise")
		f.set_ylabel('Entropy')
...
#	plt.tight_layout(pad=2)	
	...
	figname = 'PrntSetSize_vs_cycle_length_1min.png'
#	
