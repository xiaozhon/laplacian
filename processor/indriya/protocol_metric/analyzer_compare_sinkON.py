...
def compare_metric(metric_name, infname, idx_avg, idx_std, merge, f, has_legend=False):
	...
	if not merge:
		...
		bar_orw_lpl1024 = f.bar(x+2*bar_width, metric_orw_lpl1024, bar_width, color='c', yerr=errs_orw_lpl1024, label='orw_lpl1024')
...
#		plt.margins(x=0.2, y=0.2)		## margin between plotted lines to the axis
	...
	'''
	figname = 'sinkON_ctp_ctp+eer_orw_indriya_compare_entropy_noLoop_updated.png'
