...
#	outf.close()
	...
	alloutput.writelines('nodeid\tdup_percent\n')
	for i in range(N):
...
#			newline = '%d\t%.4f\n' % (nodeids[i], duped_id[nodeids[i]]*1.0 / (len(true_dup) + len(duped_pkts)))
	...
	### The percentage of the duplicates packets among all the packets a node has forwarded. 
	### node_dups / node_forwards
	alloutput.writelines('nodeid\tdup_fwd_ratio\tfwd_count\tdup_count\n')
	for i in range(N):
		if node_workload.has_key(nodeids[i]):
			
			newline = '%d\t' % nodeids[i]
			if duped_id.has_key(nodeids[i]):
				### this node has duplicates
				newline += '%.4f\t%d\t' % (duped_id[nodeids[i]] * 1.0 /node_workload[nodeids[i]], 
											duped_id[nodeids[i]])
			else:
				newline += '0\t0\t'
				
			newline += '%d\n' % (node_workload[nodeids[i]])
			alloutput.writelines(newline)
...
if __name__ == "__main__":
	...
	outf_all.writelines('test\tall_dups\tdups_unique_path\tduped_path\n')
