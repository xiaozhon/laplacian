...
def compare_metric(metric_name, infname, idx_value, merge, f, has_legend=False):
	...
	"""
	### read in the node id
	in_node = open('rctp_sinkON_DAR30/nodeids.txt', 'r')
	nodeids = []
	for line in in_node:
		s = line.split()
		if len(s) > 0:
...
#		plt.margins(x=0.2, y=0.2)		## margin between plotted lines to the axis
	...
	'''
#	figname = 'sinkON_ctp_eer_orw_node_details.png'
...
#	
#	i += 1
#	compare_metric("Dup_Src", 'dup_analysis_result.txt', 1, merge, ax[i])
#	
#	i += 1
#	compare_metric("Dup_percent", 'dup_analysis_result.txt', 1, merge, ax[i])
...
#	compare_metric("Dup_Fwd_ratio", 'dup_analysis_result.txt', 1, merge, ax[i])
	...
	figname = 'sinkON_ctp_eer_orw_node_details_Dup_Fwd_ratio.png'
	compare_metric("Dup_Fwd_ratio", 'dup_analysis_result.txt', 1, merge, ax, has_legend=True)
