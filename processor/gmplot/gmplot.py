...
class GoogleMapPlotter(object):
	...
	def __init__(self, center_lat, center_lng, zoom):
		...
		# modify the path format to the consist in Win and Linux.
		# use / to replace all \
		icon_path = os.path.join(os.path.dirname(__file__), 'markers/%s.png')
		tmp_list = list(icon_path)
		new_icon_path = ""
		for i in range(0, len(tmp_list)):
			if tmp_list[i] == '\\':
				tmp_list[i] = '/'
			new_icon_path += tmp_list[i]
		self.coloricon = new_icon_path
...
#	def marker(self, lat, lng, color='#FF0000', c=None):
	def marker(self, lat, lng, color='#FF0000', img_src=None, scaledSize=None, label=None, title=None, c=None ):
		if c:
			color = c
		...
		#
		img_src = img_src or (self.coloricon % "#FF0000")
		scaledSize = scaledSize or 40
		label = label or ""
		title = title or "default"
		#
		self.points.append((lat, lng, color[1:], img_src, scaledSize, label, title))
	...
	# ADD: XY
	# Add a point using self-designed img, scaledSize, label text, and title
	def plot_point(self, lats, lngs, img_src=None, scaledSize=None, label=None, title=None, **kwargs):
		img_src = img_src or (self.coloricon % "#FF0000")
		scaledSize = scaledSize or 40
		label = label or ""
		title = title or "default"
		kwargs["img_src"] = img_src
		kwargs["scaledSize"] = scaledSize
		kwargs["label"] = label
		kwargs["title"] = title
		settings = self._process_kwargs(kwargs)
		for lat, lng in zip(lats, lngs):
#			print img_src
			self.marker(lat, lng, settings['color'], img_src, scaledSize, label, title)
	...
	def _process_kwargs(self, kwargs):
		...
		# ADD: XY
		settings["img_src"] = kwargs.get("img_src", None)
							
		settings["scaledSize"] = kwargs.get("scaledSize", None) 
		
		settings["label"] = kwargs.get("label", None) 
	...
	def draw(self, htmlfile):
		...
		# ADD: XY, to display label
		f.write('<script src="http://cs.iupui.edu/~xiaozhon/aswp/infobox.js" type="text/javascript"></script>\n')
		
		f.write('<script type="text/javascript">\n')
		f.write('\tfunction initialize() {\n')
		self.write_map(f)
		self.write_grids(f)
		self.write_points(f)	# TODO
		self.write_paths(f)
		self.write_shapes(f)
...
#			print "point is: " + str(point)
			self.write_point(f, point[0], point[1], point[2], point[3], point[4], point[5], point[6])
	...
	def write_point(self, f, lat, lon, color=None, img_src=None, scaledSize=None, label=None, title=None,
					 size=None, marker=True, c=None, s=None,):
		
		f.write('\t\tvar latlng = new google.maps.LatLng(%f, %f);\n' %
				(lat, lon))
		
		if img_src:
			img  = 'new google.maps.MarkerImage(\'' + img_src + '\',\n'
			img += '\t\t\tnull,\n'
			img += '\t\t\tnull,\n'
			img += '\t\t\tnew google.maps.Point(4,4),\n'
			img += '\t\t\tnew google.maps.Size(' + str(scaledSize) + ',' + str(scaledSize) + '),\n'
			img += '\t\t\tnull)'

		else:
			img = 'new google.maps.MarkerImage(\'%s\')' % (self.coloricon % color)
			
		#	f.write('\t\tvar img = new google.maps.MarkerImage(\'%s\');\n' %
		#		(self.coloricon % color))
		
		# there will be a title anyway
		if title:
			i_title = title
		else:
			i_title = 'default'
		...
		f.write('\t\t\ttitle: ' + i_title + ',\n')
#		f.write('\t\ticon: img,\n')
		f.write('\t\t\tposition: latlng\n')
		f.write('\t\t});\n\n')
		
		# set icon
		f.write('\t\tmarker.setIcon(' + img + ');\n\n')
		
		
		f.write('\t\tmarker.setMap(map);\n')
		f.write('\n\n')
		
		# add lister and labels
		if title:
			f.write('\t\tvar infowindow = new google.maps.InfoWindow(); // One for each marker\n')
			f.write('\t\tvar nodeID = "' + title + '";\n')
			f.write('\t\tgoogle.maps.event.addListener(marker, \'click\', (function(marker, nodeID) {\n')
			f.write('\t\t\treturn function() {\n')
			f.write('\t\t\t\tif (infowindow.getMap()) {\n')
			f.write('\t\t\t\t\tinfowindow.close();\n')
			f.write('\t\t\t\t} else {\n')
			f.write('\t\t\t\t\tvar infoOptions = {pixelOffset: new google.maps.Size(0,15)};\n')
			f.write('\t\t\t\t\tinfowindow.setOptions(infoOptions);\n')
			f.write('\t\t\t\t\tinfowindow.setContent(nodeID);\n')
			f.write('\t\t\t\t\tinfowindow.open(map, marker);')
			f.write('\t\t\t\t}\n')
			f.write('\t\t\t};\n')
			f.write('\t\t})(marker, nodeID));\n\n')
			
		if label:
			f.write('\t\tvar iblabelText = nodeID;\n\n')
			f.write('\t\tvar myLabelOptions = { \n')
			f.write('\t\t\tcontent: iblabelText, \n')
			f.write('\t\t\tboxStyle: { \n')
			f.write('\t\t\t\tborder: "0px solid white", \n')
			...
			f.write('\t\t\tpixelOffset: new google.maps.Size(-10, 5), \n')
			f.write('\t\t\tposition: latlng, \n')
			f.write('\t\t\tcloseBoxURL: "", \n')
			f.write('\t\t\tisHidden: false, \n')
			f.write('\t\t\tpane: "mapPane", \n')
			f.write('\t\t\tenableEventPropagation: true \n')
			f.write('\t\t}; \n\n')
			f.write('\t\tvar infoboxLabel = new InfoBox(myLabelOptions); \n')
			f.write('\t\tinfoboxLabel.open(map); \n\n')

	'''	
		// Create a label text			
		var iblabelText = nodeID;
		...
		var infoboxLabel = new InfoBox(myLabelOptions);
...
#	mymap.marker(lat, lng, "red")
	...
	path = [(37.429, 37.428, 37.427, 37.427),
			 (-122.145, -122.145, -122.145, -122.146)]
