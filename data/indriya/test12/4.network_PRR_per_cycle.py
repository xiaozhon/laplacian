...
inf = open('20161031-rctp_indriya_all_packets.txt', 'r')
...
#				outf.writelines('\t' + key + ': ' + str(node_pkts[key]) + '\n')
			...
			for i in range(N):
				...
				if round_count == 177 and nodeids[i] == '135':
...
for i in range(N):
	outf.writelines(nodeids[i] + ': ' + str(prr[idx(nodeids[i])]) + '\t' + str(prnt_set[idx(nodeids[i])]) + '\n')
